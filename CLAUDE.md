...
Layne is a centralized appsec scanning GitHub App. It receives `pull_request` webhooks from GitHub, enqueues scan jobs via BullMQ/Redis, and posts results back as GitHub Check Run annotations. It runs two scanners: Semgrep (SAST) and Trufflehog (secret detection).
...
`validateEnv()` in `src/env.js` checks these at startup and exits if any are missing.
...
- `semgrep.js` — runs `semgrep scan --config auto --json --baseline-commit FETCH_HEAD`; exit code 1 = findings found (not an error); maps ERROR→high, WARNING→medium, INFO→low
...
- `src/__tests__/setup.js` sets all required env vars before each test file runs (no `.env` file needed for tests)
- External dependencies (`@octokit/auth-app`, `@octokit/rest`, `bullmq`, `ioredis`) are always mocked — no live Redis or GitHub API calls in tests
