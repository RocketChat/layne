...
import type { ProcessedFinding, ExceptionData, ParsedCommand, ExceptionApproversConfig, LineRangesByFile } from './types.js';

const EXCEPTION_TTL = 30 * 24 * 60 * 60; // 30 days in seconds

export function generateFindingId(finding: Pick<ProcessedFinding, 'tool' | 'file' | 'line' | 'startLine'>): string {
...
export async function resolveDriftedExceptions({ unmatchedFindings, owner, repo, prNumber, workspacePath, currentHeadSha }: {
  ...
  for (const [approvedHeadSha, exceptionsAtSha] of bySha) {
    ...
    for (const file of affectedFiles) {
      ...
      let lineMap: Map<number, number | null>;
      try {
