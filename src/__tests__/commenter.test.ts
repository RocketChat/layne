...
function makeOctokit({ existingComments = [] }: { existingComments?: Array<{ id: number; body: string }> } = {}) {
  ...
  (getInstallationOctokit as ReturnType<typeof vi.fn>).mockResolvedValue(octokit);
...
describe('postComment()', () => {
  ...
  describe('success conclusion — existing comment', () => {
    ...
    it('uses the success body when updating on success', async () => {
      ...
      const { body } = updateComment.mock.calls[0][0] as { body: string };
      expect(body).toContain(COMMENT_MARKER);
  ...
  describe('error handling', () => {
    it('does not throw when getInstallationOctokit rejects', async () => {
      (getInstallationOctokit as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('auth failed'));
      await expect(
    ...
    it('does not throw when createComment rejects', async () => {
      ...
      (octokit.issues.createComment as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('API down'));
