...
function finding(overrides: Partial<ProcessedFinding> = {}): ProcessedFinding {
  const line = (overrides.line ?? overrides.startLine ?? 10) as number;
  return {
    file:     'src/app.js',
    line,
    startLine: (overrides.startLine ?? line) as number,
    endLine: (overrides.endLine ?? overrides.startLine ?? line) as number,
    severity: 'high',
...
describe('buildAnnotations()', () => {
  ...
  describe('annotations', () => {
    ...
    it('maps unknown severity → notice annotation level', () => {
      const [a] = buildAnnotations([finding({ severity: 'something-unknown' as 'high' })]).annotations;
      expect(a.annotation_level).toBe('notice');
    });
