...
describe('loadScanConfig()', () => {
  ...
  let loadScanConfig: (args: { owner: string; repo: string }) => Promise<Record<string, unknown>>;
  let DEFAULT_CONFIG: Record<string, unknown>;

  beforeEach(async () => {
    vi.resetModules();
    const fsMod = await import('fs/promises');
    readFile = fsMod.readFile as unknown as ReturnType<typeof vi.fn>;
    vi.mocked(readFile).mockReset();

    const configMod = await import('../config.js');
    loadScanConfig = configMod.loadScanConfig as unknown as typeof loadScanConfig;
  ...
  it('extraArgs fully replaces the default (no concatenation)', async () => {
    ...
    const config = await loadScanConfig({ owner: 'acme', repo: 'frontend' });
    expect((config.semgrep as { extraArgs: string[] }).extraArgs).toEqual(['--config', 'p/custom']);
    expect((config.semgrep as { extraArgs: string[] }).extraArgs).not.toContain('auto');
  });
  ...
  it('preserves enabled: false through merge', async () => {
    ...
    expect((config.trufflehog as { enabled: boolean; extraArgs: string[] }).enabled).toBe(false);
    expect((config.trufflehog as { enabled: boolean; extraArgs: string[] }).extraArgs).toEqual([]);
  });
  ...
  it('repo-level notifications override $global for the same notifier key', async () => {
    ...
    expect((config.notifications as Record<string, unknown>).rocketchat).toEqual(repoRc);
  });
  ...
  it('repo-level notifications do not affect global notifiers for different keys', async () => {
    ...
    const config = await loadScanConfig({ owner: 'acme', repo: 'payments' });
    expect((config.notifications as Record<string, unknown>).slack).toEqual(globalSlack);
    expect((config.notifications as Record<string, unknown>).rocketchat).toEqual(repoRc);
  });
  ...
  it('a repo can opt out of a global notifier by setting enabled: false', async () => {
    ...
    const config = await loadScanConfig({ owner: 'acme', repo: 'frontend' });
    expect((config.notifications as Record<string, Record<string, unknown>>).rocketchat.enabled).toBe(false);
  });
  ...
  it('repo-level trigger overrides $global trigger', async () => {
    ...
    const config = await loadScanConfig({ owner: 'acme', repo: 'frontend' });
    expect((config.trigger as { on: string }).on).toBe('pull_request');
  });
  ...
  it('inherits $global comment when the repo has no comment block', async () => {
    ...
    const config = await loadScanConfig({ owner: 'acme', repo: 'frontend' });
    expect((config.comment as { enabled: boolean }).enabled).toBe(true);
  });
  ...
  it('repo-level comment overrides $global comment', async () => {
    ...
    const config = await loadScanConfig({ owner: 'acme', repo: 'payments' });
    expect((config.comment as { enabled: boolean; template: string }).enabled).toBe(false);
    expect((config.comment as { enabled: boolean; template: string }).template).toBe('global tpl'); // inherited from global
  });
  ...
  it('repo-level template overrides $global template', async () => {
    ...
    const config = await loadScanConfig({ owner: 'acme', repo: 'payments' });
    expect((config.comment as { template: string }).template).toBe('repo tpl');
  });
