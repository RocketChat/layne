...
| Pi Agent | Disabled - must opt in per repo; requires **both** `enabled: true` and a `provider` value - omitting either keeps it disabled; also requires the corresponding provider credentials in the environment |
