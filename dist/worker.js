...
import { registry, scanTotal, scanDuration, scanTimeoutsTotal, scanRetriesTotal, findingTotal, findingPlacementTotal, findingsPerScan, queueWaiting, queueActive, queueFailed, } from './metrics.js';
...
function isActionableFinding(finding) {
    if (finding.annotationEligible === false)
        return false;
...
export async function processJob(job) {
    ...
    try {
        ...
        console.error(`[worker] Scan failed for ${job.data.owner}/${job.data.repo} PR #${job.data.prNumber}:`, `${safeMessage}${retryMessage}`);
        if (finalAttempt) {
...
async function runScan(job, scanConfig) {
    const { installationId, owner, repo, cloneUrl, headSha, baseSha, baseRef, prNumber, checkRunId, } = job.data;
    ...
    try {
        ...
        const validatedFindings = await validateFindingLocations(diffFilteredFindings, { workspacePath: scanContext.repoWorkspacePath, changedFiles });
        ...
        for (const f of actionableFindings)
            f._findingId = generateFindingId(f);
        ...
        if (exceptionApprovers?.users?.length || exceptionApprovers?.teams?.length) {
            ...
            const unmatchedBlockingFindings = actionableFindings.filter(f => (f.severity === 'critical' || f.severity === 'high') && !freshExceptions.has(f._findingId));
...
function logFindingPlacement(findings, { owner, repo, prNumber }) {
    ...
    for (const finding of findings.filter(f => f.tool === 'claude')) {
        console.log(`[worker]   claude placement ${finding.file}:${finding.startLine ?? finding.line}-${finding.endLine ?? finding.line}` +
