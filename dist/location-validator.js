...
export async function validateFindingLocations(findings, { workspacePath, changedFiles = [] }) {
    if (!findings.length)
        return [];
...
function failureReasonForEvidenceStatus(status) {
    if (status === 'missing')
...
function resolveAnnotationLocation(finding, info, evidenceLocation) {
    const fallback = {
...
function validateDeclarationAnchor(lines, anchorLine, evidenceStartLine) {
    const normalizedAnchorLine = normalizePositiveInt(anchorLine);
    if (normalizedAnchorLine === null)
        return null;
    if (normalizedAnchorLine > evidenceStartLine)
        return null;
    if (normalizedAnchorLine > lines.length)
        return null;
...
function findNearestDeclarationLine(lines, startLine) {
    for (let lineNumber = startLine; lineNumber >= 1; lineNumber--) {
        if (looksLikeDeclaration(lines[lineNumber - 1] ?? ''))
            return lineNumber;
    }
    return null;
}
...
function inspectEvidence(info, evidence) {
...
function offsetsToSpan(lineOffsets, startOffset, endOffset) {
    return {
...
function applyEvidenceLocation(finding, relocated) {
