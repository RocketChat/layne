...
const SYSTEM_PROMPT = 'You are a security code reviewer. Analyse the provided source files for malicious intent: ' +
 ...
 */
export async function runClaude({ workspacePath, changedFiles, changedLineRanges = new Map(), promptFiles = [], toolConfig = DEFAULT_CONFIG.claude, }) {
...
function extractFindings(content) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const toolUse = content.find((b) => b.type === 'tool_use' && b.name === 'report_findings');
    if (!toolUse)
        return { findings: [] };
    return {
